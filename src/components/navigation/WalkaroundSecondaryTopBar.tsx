...
interface WalkaroundSecondaryTopBarProps {
  walkaroundPageId: string;
  activeTabId?: string;
  onTabClick?: (tab: { id: string; label: string }) => void;
}

// Tabs organized by walkaround sub-page
const walkaroundTabs: Record<string, SecondaryTopBarItem[]> = {
  'all-checks': [
...
const pageDisplayNames: Record<string, string> = {
  ...
  'driven-without-checks': 'Driven without Checks',
  'walkaround-settings': 'Settings',
};

export function WalkaroundSecondaryTopBar({
  walkaroundPageId,
  activeTabId = '',
  onTabClick
}: WalkaroundSecondaryTopBarProps) {
  const tabs = walkaroundTabs[walkaroundPageId] || [];
  const pageDisplayName = pageDisplayNames[walkaroundPageId] || walkaroundPageId;
